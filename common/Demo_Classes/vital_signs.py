...
class VitalSigns(PeopleTracking):
    ...
    def initVitalsPlots(self):
        ...
        for i in range(MAX_VITALS_PATIENTS):
            ...
            statusLabel = QLabel('Patient Status 1:')
            breathLabel = QLabel('Breath Rate:')
            heartLabel = QLabel('Heart Rate:')
            rangeBinLabel = QLabel('Range Bin:')

            # I commented out this portion for safe keeping of the orignal code.
            """
            ...
            """
            #Heart Rate plot
            patientDict['Heart Rate Plot'] = pg.PlotWidget()
            patientDict['Heart Rate Plot'].setBackground('w')
            patientDict['Heart Rate Plot'].showGrid(x=True,y=True)
            patientDict['Heart Rate Plot'].invertX(True)

            #Resipiratory Rate plot
            patientDict['Respiratory Rate Plot'] = pg.PlotWidget()
            patientDict['Respiratory Rate Plot'].setBackground('w')
            patientDict['Respiratory Rate Plot'].showGrid(x=True,y=True)
            patientDict['Respiratory Rate Plot'].invertX(True)


            if(self.xWRLx432 == 1):
                patientDict['Respiratory Rate Plot'].setXRange(0, NUM_VITALS_FRAMES_IN_PLOT_IWRL6432, padding=0.01)
                patientDict['Heart Rate Plot'].setXRange(0, NUM_VITALS_FRAMES_IN_PLOT_IWRL6432, padding=0.01)

                patientDict['Heart Rate Plot'].setYRange(0,120,padding=0.1)
                patientDict['Respiratory Rate Plot'].setYRange(0,120,padding=0.1)

                patientDict['Heart Rate Plot'].getPlotItem().setLabel('left', 'Hear Rate per minute')
                patientDict['Respiratory Rate Plot'].getPlotItem().setLabel('left', 'Breath Rate per minute')

                patientDict['plot'].getPlotItem().setLabel('bottom', 'Vital Signs Frame Number')
            else:
                patientDict['Heart Rate Plot'].setXRange(0,NUM_VITALS_FRAMES_IN_PLOT,padding=0.01)
                patientDict['Respiratory Rate Plot'].setXRange(0,NUM_VITALS_FRAMES_IN_PLOT,padding=0.01)

                patientDict['Heart Rate Plot'].setYRange(-1,1,padding=0.1)
                patientDict['Respiratory Rate Plot'].setYRange(-1,1,padding=0.1)
                
            patientDict['Heart Rate Plot'].setMouseEnabled(False,False)
            patientDict['Respiratory Rate Plot'].setMouseEnabled(False,False)
            ...
            patientDict['Heart Rate Plot'].addItem(patientDict['heartGraph'])
            patientDict['Respiratory Rate Plot'].addItem(patientDict['breathGraph'])
            ...
            patientPaneLayout.addWidget(patientDict['Heart Rate Plot'],2,0,1,4)
            patientPaneLayout.addWidget(patientDict['Respiratory Rate Plot'],3,0,1,4)
